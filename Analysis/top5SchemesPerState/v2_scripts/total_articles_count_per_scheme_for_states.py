...
parser.add_argument('--output', nargs='?', default='output_graphs/v2/State Level/', help='path where the output to be stored')
args = parser.parse_args()
path = args.path
output = 'output_graphs/v2/State Level/'
...
def plot(states, keywords, coll, scheme_name, total, state_type):
    ...
    plt.tight_layout()
    path = output + coll + '/' + state_type + '/'
    make_dir(path)
    plt.savefig(path + scheme_name+'0.png')
    plt.clf()
