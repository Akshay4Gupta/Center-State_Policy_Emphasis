...
def processArticles(cacheName, art_collection, startdate, enddate, typ): 
	...
	try:
		for article in tqdm(cursor):
			...
			for name, (_, details) in MP.items():
				try:
					entity_name, state, party = details
				except Exception as e:
					continue
...
if __name__ == '__main__':
	...
	for i in range(total_elections-1):
		...
		for scheme_name in scheme_names:
			...
			article_sentiments = processArticles(cacheName, art_collection, split_dates[i], split_dates[i+1], typ)
			...
			# width = 0.25
			# party_map = {'UPA': 0, 'NDA': 1, 'NonAligned': 2, 'ThirdFront': 2, 'NotAligned': 2}
